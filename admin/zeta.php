...
$parti = $db->getParticipaciones($_GET['es']);
  ...
  foreach($parti as $pp) {
	   echo utf8_decode($pp["persona"]) . "\t" . $pp["email"]. "\t" . $pp["telefono"]. "\t" . utf8_decode($pp["direccion"]). "\t" .$pp["organiza"]. "\t" .$pp["masinfo"]. "\t" . $pp["provincia"]. "\t" . utf8_decode($pp["lugar"]). "\t". $pp["coordenadas"]. "\t" .     $pp["hora"]. "\t" . $pp["numerop"]. "\t" . utf8_decode($pp["comentarios"]). "\n";
  }
?>
